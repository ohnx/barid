...
void mail_set_allowed(struct barid_conf *conf) {
    ...
    allowed_accounts = conf->accounts;
}
...
int mail_addattr(struct mail *email, enum mail_attr attr, const char *data) {
    ...
    case TO:
        ...
        if (atpos-data == 1) return MAIL_ERROR_INVALIDEMAIL;

        /* ensure valid domain (*allowed_hosts = '*' = wildcard) */
        if (*allowed_hosts != '*' && !mail_checkinlist(atpos+1, allowed_hosts, data_len-(atpos-data)-3)) {
            return MAIL_ERROR_USRNOTLOCAL;
        }

        /* ensure valid account (*allowed_accounts = '*' = wildcard) */
        if (*allowed_accounts != '*' && !mail_checkinlist(data+1, allowed_accounts, atpos-data-1)) {
            return MAIL_ERROR_USRNOTLOCAL;
        }
